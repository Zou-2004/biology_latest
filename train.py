...
class IM_AE(object):
    def __init__(self, config):
        ...
        # Initialize network
        self.network = INR_Network(self.ef_dim, self.gf_dim, self.z_dim, self.point_dim).to(self.device_id)
        self.network = DDP(
            self.network,
        ...
        self.optimizer = optim.Adam([
            ...
            {'params': z_params, 'lr': self.scaled_lr * 0.01}  # Very small learning rate for z
        ])
    ...
    def combined_loss(self, predicted_density, true_density):
        ...
        # Clamp weights to max 50
        weights = torch.clamp(weights, max=50.0)
        # Re-normalize after clamping to maintain the average weight of 1
        weights = weights / weights.mean()
        ...
        with h5py.File(data_file, 'r') as f:
            ...
            for file_idx, file_name in enumerate(file_names):
                ...
                if self.rank == 0:
                    ...
                    print(f"File {file_name} completed: Avg Loss: {file_avg_loss:.6f}, Max Loss: {max_loss:.6f}")
